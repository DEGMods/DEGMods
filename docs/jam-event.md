...
- **Its kind matches the jam's type (`j`).** A mod jam (`j=mod`) counts only mod entries (`31142`); a game jam (`j=game`) counts only game entries (the future game kind). This stops a mod being counted in a game jam's leaderboard (or vice-versa) if some client tags it that way. Enforced by `isValidSubmission` via a `jamType → kind` map; until the game kind exists, a game jam matches no kind and so counts nothing.
...
- **Entries** are the future **game kind** (see below) instead of `31142`, still linked with `["a","31143:…"]` + `["l","jam-entry"]`. Game releases reuse the `l` slot for their own types (`"demo"`, `"dlc"`, `"playtest"`, …), independent of jams.
  - The kind↔type match is **already enforced**: add `game: KINDS.GAME` to the `JAM_ENTRY_KIND` map in `jam.ts` when the kind is defined, and `isValidSubmission` immediately accepts game entries in game jams and keeps rejecting cross-type ones. Nothing else changes.
